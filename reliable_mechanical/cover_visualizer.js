...
files = [
	...
	['reliable mechanical bag 2 cover.csv'], // bag 2
	[
		'reliable bag 3 STSD path cover.csv'], // bag 3
    [
		'reliable mechanical bag 4 STSD path cover.csv'], // bag 4
    [
...
function loadIncludedFile(bag_num) {
	...
	const url = window.location.href.replace('index.html', '').replace('/reliable_mechanical', '') + 'cover_csvs/' + filename;
	console.log(url);
	fetch(url)
		.then((r) => r.text())
		.then((t) => {
