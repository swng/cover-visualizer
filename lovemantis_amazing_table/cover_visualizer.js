...
files = [
	['amazing table bag 1 cover.csv'], // bag 1
	['amazing table bag 2 cover.csv'], // bag 2
	['amazing table bag 3 cover.csv'], // bag 3
    [
		'amazing table bag 4 path a cover.csv',
		'amazing table bag 4 path b cover.csv',
    ], // bag 4
    [
		'amazing table bag 5 path a cover.csv',
        'amazing table bag 5 path b cover.csv',
    ], // bag 5
];
...
document.getElementById('mirror').addEventListener('change', (e) => {
	...
	if (e.target.checked) {
		...
		for (bag_num = 1; bag_num < 5; bag_num++) {
			container = document.getElementById(`setup ${bag_num} preview`);
			fumenrender(mirrorFumen([setup[bag_num - 1]]), container);
		}
	} else {
